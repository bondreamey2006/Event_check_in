...
When route and service tests are present, run them from `backend/` with:

```bash
pytest
```

Run the separate multi-process concurrency proof with `python scripts/load_test.py --base-urls http://localhost:8000 http://localhost:8001` as described in [Multi-process proof](#multi-process-proof).

## Deployment

Deploy the three tiers independently:

| Tier | Recommended host | Deployment notes |
| :--- | :--- | :--- |
| **PostgreSQL** | Render PostgreSQL (or another managed PostgreSQL service) | Create a production database, enable backups, and provide its private `DATABASE_URL` only to the backend. Apply migrations before serving traffic. |
| **FastAPI + Socket.IO backend** | Render Web Service | Deploy from `backend/` with a production ASGI command such as `uvicorn app.main:app --host 0.0.0.0 --port $PORT`. Set `DATABASE_URL`, `LLM_API_KEY`, and `FRONTEND_ORIGINS` in the host's secret environment configuration. Configure the service for WebSocket support. |
| **Vite frontend** | Vercel | Build from `frontend/` with `npm run build`. Set the public API and Socket.IO base URLs to the deployed backend URL, then redeploy after changing either value. |

For production, set `FRONTEND_ORIGINS` to the exact Vercel domain (and any approved custom domain), use HTTPS for every public endpoint, and never expose `DATABASE_URL` or `LLM_API_KEY` in a frontend variable or build artifact.

## Code quality

Keep production Python and React source concise, consistently formatted, and free of explanatory inline comments. This README, endpoint contracts, focused tests, and clear names should document intent; code should make the execution path clear on its own.
